...
flags.DEFINE_string(
    "output_dir",
    "/tmp/music_vae/generated",
    "The directory where MIDI files will be saved to.",
)
flags.DEFINE_string("config", None, "The name of the config to use.")
flags.DEFINE_string(
    "mode", "sample", "Generate mode (either `sample` or `interpolate`)."
)
flags.DEFINE_string("input_midi_1", None, "Path of start MIDI file for interpolation.")
flags.DEFINE_string("input_midi_2", None, "Path of end MIDI file for interpolation.")
...
flags.DEFINE_float("temperature", 0.5, "The randomness of the decoding process.")
flags.DEFINE_string(
    "log",
    "INFO",
...
def run(config_map):
    ...
    if FLAGS.mode == "interpolate":
        ...
        results = model.decode(
            length=config.hparams.max_seq_len, z=z, temperature=FLAGS.temperature
        )
