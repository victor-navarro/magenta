...
"""MusicVAE training script with optional distributed runtime wiring."""
...
flags.DEFINE_string("master", "", "The TensorFlow master to use.")
flags.DEFINE_string(
    "worker_hosts", "", "Comma-separated list of worker host:port addresses."
)
flags.DEFINE_string(
    "ps_hosts", "", "Comma-separated list of parameter-server host:port addresses."
)
flags.DEFINE_string("job_name", "", "Distributed job name: `worker` or `ps`.")
flags.DEFINE_integer("task_index", 0, "Distributed task index for the selected job.")
flags.DEFINE_string(
    "examples_path",
    None,
...
flags.DEFINE_string(
    "tfds_name", None, "TensorFlow Datasets dataset name to use. Overrides the config."
)
flags.DEFINE_string(
    "run_dir",
    None,
...
flags.DEFINE_integer(
    "num_steps", 200000, "Number of training steps or `None` for infinite."
)
flags.DEFINE_integer(
    "eval_num_batches",
    None,
...
flags.DEFINE_string("mode", "train", "Which mode to use (`train` or `eval`).")
flags.DEFINE_string("config", "", "The name of the config to use.")
flags.DEFINE_string(
    "hparams",
    "",
...
flags.DEFINE_integer("task", 0, "The task number for this worker.")
flags.DEFINE_integer("num_ps_tasks", 0, "The number of parameter server tasks.")
flags.DEFINE_integer("num_sync_workers", 0, "The number of synchronized workers.")
flags.DEFINE_string("eval_dir_suffix", "", "Suffix to add to eval output directory.")
flags.DEFINE_string(
    "log",
    "INFO",
...
def _trial_summary(hparams, examples_path, output_dir):
    ...
    examples_path_summary = tf.summary.text(
        "examples_path",
        tf.constant(examples_path, name="examples_path"),
        collections=[],
    )
...
def train(
    ...
    with tf.Graph().as_default():
        with tf.device(
            ...
            model = config.model
            model.build(
                config.hparams, config.data_converter.output_depth, is_training=True
            )
...
def evaluate(train_dir, eval_dir, config, dataset_fn, num_batches, master=""):
    ...
    with tf.Graph().as_default():
        model = config.model
        model.build(
            config.hparams, config.data_converter.output_depth, is_training=False
        )
...
def run(
    ...
    """
    # Log availabel GPUs for debugging purposes.
    local_device_protos = tf.config.list_logical_devices()
    gpu_devices = [d for d in local_device_protos if d.device_type == "GPU"]
    tf.logging.info("Available GPU devices: %s", gpu_devices)
    ...
    runtime = _maybe_start_distributed_runtime()
    if runtime == "joined":
        return

    effective_master = FLAGS.master
    effective_task = FLAGS.task
    effective_num_ps_tasks = FLAGS.num_ps_tasks
    effective_num_sync_workers = FLAGS.num_sync_workers
    if runtime:
        effective_master = runtime["master"]
        effective_task = runtime["task"]
        effective_num_ps_tasks = runtime["num_ps_tasks"]
        effective_num_sync_workers = runtime["num_sync_workers"]
    ...
    if effective_num_sync_workers:
        config.hparams.batch_size //= effective_num_sync_workers
    ...
    def dataset_fn():
        ...
        if is_training and effective_num_sync_workers:
            # Ensure workers read disjoint batches during data-parallel training.
            dataset = dataset.shard(effective_num_sync_workers, effective_task)
        return dataset
    ...
    if is_training:
        train(
            ...
            num_sync_workers=effective_num_sync_workers,
            num_ps_tasks=effective_num_ps_tasks,
            task=effective_task,
        )
    else:
        num_batches = (
            FLAGS.eval_num_batches
            or data.count_examples(
